...
class AuthenticateAPIOnce
{
    ...
    public function handle(Request $request, \Closure $next)
    {
        ...
        // new UserProvider;
        $a = new AccessTokenGuard($request, []);
        $z = $a->validate(['access_token' => $authKey]);dd($z);
        //dd($authKey);//return $next($request);
        $matches = [];
        if (\preg_match("/" . self::AUTH_HASH_PREFIX . "(\w+)/", $authKey, $matches)) {
            $hash = $matches[1];dd($data);
            $timestamp = Carbon::now()->getTimestamp();
            $text = !empty($data) ? static::dataToString($data) : '';
            for ($i = 0; $i < self::DELAY_SECOND; $i++) {
                $hash1 = static::genTokenHash($text . ($timestamp - $i), Util::getApiSaltKey());dd($hash1);
                if (strpos($hash1, $hash) !== false) {dd('3');
                    return $next($request);
                }
            }
        }dd('4');
